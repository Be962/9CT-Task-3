...
def filter():
    ...
    year = df.iloc[start_years:end_years, 6]
...
def show_other_stats():
    ...
    overall_rpo = df['Runs'].sum() / df['Balls'].sum()
    overall_rpo = overall_rpo * 6
    overall_rpo = overall_rpo.round(4)
    return stddf, overall_average, overall_rpo
